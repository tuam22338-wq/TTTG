...
const CodexPanel: React.FC<CodexPanelProps> = ({ isOpen, onClose, gameState, onUpdateRule, onAddRule, onDeleteRule, onStatClick }) => {
  ...
  const allLoreAndRules = useMemo(() => [
    ...gameState.worldContext.initialLore,
    ...gameState.worldContext.specialRules,
    ...gameState.codex,
  ].sort((a,b) => a.name.localeCompare(b.name)), [gameState]);

  const dynamicCategories = useMemo(() => {
    const tags = new Set<string>();
    allLoreAndRules.forEach(entry => {
  ...
  const allItems = useMemo(() => {
    const itemsInGame = [
        ...
        ...Object.values(gameState.equipment).filter((item): item is Item => item !== null)
    ];
    const uniqueItems = Array.from(new Map(itemsInGame.map(item => [item.id, item])).values());
    return uniqueItems.sort((a, b) => a.name.localeCompare(b.name));
  }, [gameState.inventory.items, gameState.equipment]);

  const filteredLore = useMemo(() => {
    let source = allLoreAndRules;
    if (activeTab !== 'all_lore') {
        source = source.filter(item => item.tags?.includes(activeTab));
    }
    return source.filter(item => 
        item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.content.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [allLoreAndRules, searchTerm, activeTab]);
  ...
  const currentEntryData = useMemo(() => {
      ...
      switch(selectedEntry.type) {
          ...
          default: source = allLoreAndRules; break; // Covers all lore/rule tabs
      }
      return source.find(item => item.id === selectedEntry.id) || null;
  }, [selectedEntry, allItems, allLoreAndRules]);

  const handleSelectEntry = (type: string, id: string) => {
      setSelectedEntry({ type, id });
  };
  
  const NavButton: React.FC<{ tabId: string, icon: React.ReactNode, label: string }> = ({ tabId, icon, label }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs mt-1 font-semibold truncate">{label}</span>
      </button>
    );
  };
  ...
  const isLoreTabActive = activeTab === 'all_lore' || dynamicCategories.includes(activeTab);
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-40 p-4 animate-fade-in-fast" onClick={onClose}>
      ...
      >
        ...
        <nav className="w-24 flex-shrink-0 bg-black/20 border-r border-neutral-700/50 p-3 flex flex-col items-center gap-3">
            ...
            <div className="w-full border-b border-neutral-700 my-1"></div>
            <NavButton tabId="all_lore" label="Tất cả" icon={<BookIcon className="h-full w-full"/>} />
            {dynamicCategories.map(cat => (
                 <NavButton key={cat} tabId={cat} label={cat} icon={<div className="h-full w-full flex items-center justify-center text-lg font-bold">#</div>} />
            ))}
        </nav>
            ...
            <aside className="w-80 flex-shrink-0 border-r border-neutral-700/50 flex flex-col bg-black/10">
                 ...
                 <div className="flex-grow overflow-y-auto p-2 custom-scrollbar">
                    <div className="space-y-1">
                        {(isLoreTabActive ? filteredLore : filteredItems).map(entry => (
                             <button key={entry.id} onClick={() => handleSelectEntry(activeTab, entry.id)} className={`w-full text-left p-3 rounded-lg transition-colors duration-200 relative overflow-hidden ${selectedEntry?.id === entry.id ? 'bg-pink-900/40' : 'hover:bg-neutral-800/60'}`}>
                                {selectedEntry?.id === entry.id && <div className="absolute left-0 top-0 h-full w-1 bg-pink-400"></div>}
                                <p className={`font-bold truncate pl-2 ${selectedEntry?.id === entry.id ? 'text-pink-300' : 'text-neutral-200'}`}>{('name' in entry) ? entry.name : '(Chưa có tên)'}</p>
                            </button>
                        ))}
                    </div>
                 </div>
            ...
            <main className="flex-grow p-6 sm:p-8 overflow-y-auto custom-scrollbar">
                 {!currentEntryData ? (
                    ...
                    <div className="space-y-6 animate-fade-in-fast">
                        ...
                        {(currentEntryData as WorldRule).tags && ((currentEntryData as WorldRule).tags?.length ?? 0) > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {((currentEntryData as WorldRule).tags ?? []).map(tag => (
                                    <span key={tag} className="px-2 py-1 text-xs font-semibold bg-pink-900/50 text-pink-300 rounded-full">{tag}</span>
                                ))}
                            </div>
                        )}
